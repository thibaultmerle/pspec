...
def load_fits(finp, verb, extn):
   ...
   try:
      ...
      print "Input filename does not exist or is not in FITS format."; quit(1)
...
def broadgauss(x, y, sigma):
   ...
   if plot:
      pl.figure(2); pl.plot(xk,yk,'+-'); pl.show()
...
def linear_norm(x, y, msk, eps=0.003, deps=0.001, nmin=2, nwin=3):
   ...
   if pl.any(w != w): 
      print "Pb with linalg.lstsq. Try to reduce eps or nmin."; quit(1)
...
def select_data(i, data, lbdmin, lbdmax, obslist, lbdunitlist, hshiftlist):
   ...
   if bla:
      ...
      if hshift != 0.0: print " Including a shift of:       ", hshift.__format__('9.3f')
...
#         llname = ll0name
   ...
   # Figure size option
   if pl.size(figsize) == 2: figsize_x, figsize_y = figsize[0], figsize[1] 
   elif pl.size(figsize) == 1: figsize_x, figsize_y = figsize[0], figsize[0]
   else:
